...
"""
This code is used to fit the observed SEDs of stars with model spectra.
It is based on the Blackbody fitting results (Output_BlackBody_Fits.csv) generated 
by 4_BlackBodyFit.py and the model photometry density grids pre-calculated.
It perform multi-step fitting analysis on the observed photometry data of the target stars, 
and ultimately obtain optimized physical parameters and luminosity distribution.

The code is divided into two parts:
Part 1: SED fitting using the original grid of model spectra, i.e., without interpolating.
Part 2: Interpolate model flux densities.

Input:
1. file_input_params: a csv file containing the stellar parameters of the target stars.
2. file_input_photometry: a csv file containing the observed photometry data of the target stars.
3. Output_BlackBody_Fits.csv: a csv file containing the blackbody fitting results of the target stars.
4. PassbandFlux_*.csv: a csv file containing the pre-calculated model flux densities.
5. filter_zero_point_path: a csv file containing the zero points of the photometric bands.
"""
...
for ii in range(index_start, index_end):  
    ...
    if LoffGrid==False:
        ...
        ax[4].grid(visible=True, which='minor', zorder=1)
        ax[4].grid(visible=True, which='major', zorder=2)
