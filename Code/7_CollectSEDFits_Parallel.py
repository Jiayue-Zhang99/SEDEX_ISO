...
"""
This script is used to collect the SED fitting results obtained from the SED fitting code.
The results are collected in two modes: (1) without the model interpolation, and (2) with the model interpolation.

The results are saved in the following files:
    1. Results table in different modes:
        - `Output_SED_Fits_No_Interp.csv`: the SED fitting results without the model interpolation.
        - `Output_SED_Fits_Interp.csv`: the SED fitting results with the model interpolation.
        - `Output_SED_Fits_Interp_MOD.csv`: the SED fitting results with the model interpolation, but the model parameters are estimated from the Gaussian fitting.
    2. Other files:
        - `Output_SED_Fits_bandsUsed.csv`: the passbands used for the SED fitting.
    3. Final results:
        - `Output_SED_Fits_Final.csv`: the final SED fitting results after rejecting low-quality results.
"""
