...
"""
This code is mainly used to calculate the flux density and bolometric flux of 
MARCS model spectrum data through filter transmission curves. 
Finally, the results are saved as files for subsequent scientific analysis or model fitting.
"""

def extract_params(chemcomp="st"):
    """
    extract physical parameters from the filenames of MARCS models,
    such as effective temperature (Teff), surface gravity (logg), metallicity ([Fe/H]), mass, etc.
    input: chemcomp parameter, used to select chemical composition, default is "st" (standard chemical composition).
    output: a DataFrame containing the model filename and the corresponding physical parameters.
    """
    # each filename has the exactly same length.
    models = pd.read_csv(grid_list_path+"AllModels.list", names=["filename"])
...
def loop_calFluxDensity(ind):
    """
    calculate the flux density through each filter for the specified model.
    convolve the model spectrum with the filter transmission curve to calculate the response of each filter.
    input: the index value ind of the current model.
    output: the filename of the current model and the flux density through all filters (in log scale).
    """
    ...
    spec_ori = readspec(grid_spectrum_path, Fluxes.loc[ind, "filename"])
...
def loop_calFbol(ind):
    """
    calculate the bolometric flux for the specified model.
    """
    print("#{:d} calculate bolemetric fluxes for the model: {:s}".format(ind, models.loc[ind, "filename"]))
    # read in the wavelength and flux of MARCS models.
    spec_ori = readspec(grid_spectrum_path, models.loc[ind, "filename"])
...
model_avail = pd.DataFrame({"filename": map(os.path.basename, np.sort(glob.glob(grid_spectrum_path+"*st*.flx")))})
...
models[["teff","logg","feh","tv","mass","modelindex", "fbol"]].to_csv(grid_list_path+"StandardComposition_SphericalPlane.txt", index=False)
...
Fluxes.to_csv(grid_list_path+"PassbandFlux_MARCS.csv",index=False, float_format="%.8f")
